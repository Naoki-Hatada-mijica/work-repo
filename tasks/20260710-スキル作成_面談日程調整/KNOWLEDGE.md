...
- 面談調整ブロックの透過設定は後日修正（下記「確保しつつ重複OK」設計を参照）。色分け：調整=バナナ(5)／確定=トマト(11)／バッファ=グラファイト(8)。
...
## 面談調整ブロックの「確保しつつ重複OK」設計（2026-07-10 修正）
- 当初 transparent（空き時間表示）で作ったが、それだと**面談調整枠が空きに見えて他予定が入る余地**が残り NG。
- 正しい設計：
  - 面談調整ブロックは**不透明（opaque）**で作成し、時間をしっかり確保する（他予定が入らない）。
  - 空き計算（`gcal_common._busy_intervals`）は **events.list ベース**にし、
    「面談調整｜」で始まる予定と transparent 予定を **busy から除外**する。
    → 面談調整同士は重複OK（同じ枠を複数候補者へ並行提案可）、確定面談・通常予定はブロック。
- freebusy だけでは予定名で除外できないため events.list へ切替えたのがポイント。
- 実カレンダーで検証済み：候補Aの面談調整枠に対し、候補Bの下見で同枠が空きとして出る／freebusy 上は busy。
